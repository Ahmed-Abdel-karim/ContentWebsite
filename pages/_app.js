...
function MyApp({ Component, pageProps }) {
  ...
  useEffect(() => {
    async function loadData() {
      if (window.localStorage.getItem("NavDataListTime") != null) {
        if (
          new Date(
            window.localStorage.getItem("NavDataListTime")
          ).getMinutes() +
            5 <
          new Date().getMinutes()
        ) {
      ...
      if (window.localStorage.getItem("NavDataList") != null && !flag) {
        ...
        const NavDataList = await fetch(
          "https://swagger.pioneer.city-edge-developments.com/api/Home/ListNavbar",
          {
            method: "get",
            headers: {
      ...
      if (window.localStorage.getItem("LogoIconTime") != null) {
        if (
          new Date(window.localStorage.getItem("LogoIconTime")).getMinutes() +
            5 <
          new Date().getMinutes()
        ) {
      ...
      if (window.localStorage.getItem("LogoIcon") != null && !flag) {
        ...
        const LogoIcon = await fetch(
          "https://swagger.pioneer.city-edge-developments.com/api/Home/GetLogo",
          {
            method: "get",
            headers: {
              LanguageCode: lang == "ar" ? "ar" : "en",
      ...
      if (window.localStorage.getItem("contactDataTime") != null) {
        if (
          new Date(
            window.localStorage.getItem("contactDataTime")
          ).getMinutes() +
            5 <
          new Date().getMinutes()
        ) {
      ...
      if (window.localStorage.getItem("contactData") != null && !flag) {
        ...
        const contactData = await fetch(
          "https://swagger.pioneer.city-edge-developments.com/api/Home/GetHomeContact",
          {
            method: "get",
            headers: {
              LanguageCode: lang == "ar" ? "ar" : "en",
        ...
        window.localStorage.setItem(
          "contactData",
          JSON.stringify(Listofcontact.data)
        );
      ...
      if (window.localStorage.getItem("socialDataTime") != null) {
        if (
          new Date(window.localStorage.getItem("socialDataTime")).getMinutes() +
            5 <
          new Date().getMinutes()
        ) {
      ...
      if (window.localStorage.getItem("socialData") != null && !flag) {
        ...
        const socialData = await fetch(
          "https://swagger.pioneer.city-edge-developments.com/api/Home/ListSocailMedia",
          {
            method: "get",
            headers: {
              LanguageCode: lang == "ar" ? "ar" : "en",
        ...
        window.localStorage.setItem(
          "socialData",
          JSON.stringify(listOfSocial.data)
        );
      ...
      if (window.localStorage.getItem("footerLinksTime") != null) {
        if (
          new Date(
            window.localStorage.getItem("footerLinksTime")
          ).getMinutes() +
            5 <
          new Date().getMinutes()
        ) {
      ...
      if (window.localStorage.getItem("footerLinks") != null && !flag) {
        setlistfooterLinks(
          JSON.parse(window.localStorage.getItem("footerLinks"))
        );
      } else {
        const footerLinks = await fetch(
          "https://swagger.pioneer.city-edge-developments.com/api/Home/ListFooterLinks",
          {
            method: "get",
            headers: {
              LanguageCode: lang == "ar" ? "ar" : "en",
        ...
        window.localStorage.setItem(
          "footerLinks",
          JSON.stringify(ListFooter.data)
        );
      ...
      if (window.localStorage.getItem("usFooterTime") != null) {
        if (
          new Date(window.localStorage.getItem("usFooterTime")).getMinutes() +
            5 <
          new Date().getMinutes()
        ) {
      ...
      if (window.localStorage.getItem("usFooter") != null && !flag) {
        ...
        const footerUs = await fetch(
          "https://swagger.pioneer.city-edge-developments.com/api/Home/GetAboutUsFooter",
          {
            method: "get",
            headers: {
              LanguageCode: lang == "ar" ? "ar" : "en",
    ...
    navData &&
      navData.map((category) => {
        category.cities.map((item) => {
          window.localStorage.setItem(
            `${item.name.replace(" ", "-")}-${category.propertyUrl}`,
            item.id
          );
        });
      });
  ...
  const whatssApp = () => {
    if (Object.keys(link).length === 0 && link.constructor === Object) {
      window.open(
        `https://api.whatsapp.com/send?phone=2${contact.mobileNumber}&text=${
          lang == "ar"
      ...
      window.open(
        `https://api.whatsapp.com/send?phone=2${contact.mobileNumber}&text=${
          lang == "ar" ? "مرحبا لدي استفسار عن" : "Hey, I have a question about"
        } ${Object.values(link)}`
      );
    }
  };
    ...
    return (
      <div className="mybody" dir={lang == "ar" ? "ltr" : "rtl"}>
        ...
        <Footer
          ...
          footerUsObj={footerUsObj}
          whatssApp={whatssApp}
          logo={mylogo.footerLogo}
          lang={lang}
        />
      </div>
    );
