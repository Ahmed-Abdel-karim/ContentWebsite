  ...
  useEffect(() => {
    async function loadData() {
      if (window.localStorage.getItem("devssliderTime") != null) {
        if (
          new Date(window.localStorage.getItem("devssliderTime")).getMinutes() +
            5 <
          new Date().getMinutes()
        ) {
      ...
      if (window.localStorage.getItem("devsslider") != null && !flag) {
        ...
        const developersList = await fetch(
          "https://swagger.pioneer.city-edge-developments.com/api/Home/ListDeveloper",
          {
            method: "get",
            headers: {
